...
public static class VanillaItemInstancer
{
    ...
    public static AudioClip GetAudioClip(string audioClipName, string defaultAudioClipName)
    {
        ...
        if (audioClip != null)
        {
            //Debug.Log($"Returning audioclip {audioClipName}");
            return audioClip;
        }
    ...
    public static GrabbableObject AddGrabbableObject(Scrap scrap, Item item)
    {
        ...
        DebugHelper.LogWarning($"Unknown scrap type '{(int)scrap.scrapType}' for scrap '{scrap.itemName}'");
        return scrap.prefab.AddComponent<PhysicsProp>();
    }
